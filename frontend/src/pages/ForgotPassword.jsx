...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await getCsrfCookie();
      // standard laravel breeze path
      const response = await api.post('/auth/forgot-password', { email: email.trim() });
      setStatus(response.data.status || 'We have emailed your password reset link!');
    } catch (err) {
      console.error(err);
      if (err.response && err.response.data && err.response.data.message) {
        setError(err.response.data.message);
      } else {
        setError('Failed to send reset link. Ensure server is online.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] flex items-center justify-center px-6">
      <div className="max-w-md w-full bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-10 rounded-[2.5rem] shadow-xl text-left">
        ...
        <h2 className="text-3xl font-serif text-slate-900 dark:text-white mb-2 leading-tight">Recover Password</h2>
        <p className="text-xs text-slate-450 dark:text-slate-400 mb-8 leading-relaxed">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#4E7D5B] text-white py-5 rounded-full text-[11px] font-black uppercase tracking-[0.3em] shadow-xl shadow-[#4E7D5B]/20 hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 cursor-pointer animate-pulse"
          >
        ...
        <div className="mt-8 text-center">
          <Link to="/login" className="text-[10px] font-black text-[#4E7D5B] hover:underline uppercase tracking-widest">
            Back to Sign In
          </Link>
        </div>
      </div>
