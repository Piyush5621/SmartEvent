<x-organizer-layout>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto">
            <form action="{{ route('organizer.events.coupons.update', [$event, $coupon]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                    <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                        <h2 class="text-lg font-bold text-slate-800 flex items-center">
                            <i data-lucide="ticket" class="w-5 h-5 mr-2 text-indigo-500"></i> Edit Coupon
                        </h2>
                        <span class="font-mono font-bold text-slate-800 bg-white px-3 py-1 rounded shadow-sm border border-slate-200">{{ $coupon->code }}</span>
                    </div>
                    ...
                    <div class="p-8 space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" step="0.01" name="value" id="value" required value="{{ old('value', $coupon->value) }}"
                                    class="w-full border-slate-200 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 py-3">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-slate-100">
                            <div>
                                ...
                                <input type="number" step="0.01" name="max_discount" id="max_discount" value="{{ old('max_discount', $coupon->max_discount) }}" placeholder="Optional"
                                    class="w-full border-slate-200 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 py-3">
                                <p class="text-[11px] text-slate-500 mt-1">Useful for percentage discounts</p>
                            </div>

                            <div>
                                <label for="min_order_amount" class="block text-sm font-semibold text-slate-700 mb-1">Minimum Order Amount (₹)</label>
                                <input type="number" step="0.01" name="min_order_amount" id="min_order_amount" value="{{ old('min_order_amount', $coupon->min_order_amount) }}" placeholder="Optional"
                                    class="w-full border-slate-200 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 py-3">
                            </div>

                            <div>
                                <label for="usage_limit" class="block text-sm font-semibold text-slate-700 mb-1">Total Usage Limit</label>
                                <input type="number" name="usage_limit" id="usage_limit" value="{{ old('usage_limit', $coupon->usage_limit) }}" placeholder="Unlimited if empty"
                                    class="w-full border-slate-200 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 py-3">
                            </div>

                            <div>
                                <label for="usage_per_user" class="block text-sm font-semibold text-slate-700 mb-1">Limit Per User</label>
                                <input type="number" name="usage_per_user" id="usage_per_user" required value="{{ old('usage_per_user', $coupon->usage_per_user) }}"
                                    class="w-full border-slate-200 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 py-3">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-slate-100">
                            <div>
                                ...
                                <input type="datetime-local" name="valid_from" id="valid_from" required value="{{ old('valid_from', $coupon->valid_from ? $coupon->valid_from->format('Y-m-d\TH:i') : '') }}"
                                    class="w-full border-slate-200 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 py-3">
                            </div>

                            <div>
                                <label for="valid_until" class="block text-sm font-semibold text-slate-700 mb-1">Valid Until</label>
                                <input type="datetime-local" name="valid_until" id="valid_until" required value="{{ old('valid_until', $coupon->valid_until ? $coupon->valid_until->format('Y-m-d\TH:i') : '') }}"
                                    class="w-full border-slate-200 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 py-3">
                            </div>
                        </div>
                        ...
                        <div class="pt-6 border-t border-slate-100 space-y-4">
                            <label class="flex items-center gap-3 cursor-pointer p-4 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $coupon->is_active) ? 'checked' : '' }} class="w-5 h-5 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500">
                                <div>
                            ...
                            <label class="flex items-center gap-3 cursor-pointer p-4 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
                                <input type="checkbox" name="apply_to_all_events" value="1" {{ old('apply_to_all_events', $coupon->event_id === null) ? 'checked' : '' }} class="w-5 h-5 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500">
                                <div>
